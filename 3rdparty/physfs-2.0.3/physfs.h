...
/**
 * \fn int PHYSFS_getMemfileName(const PHYSFS_memfile *memfile)
 * \brief Generate an archive name to reference a memfile pointer
 *
 * You give this function the pointer of a memfile object, and it generates
 *  an archive name (that may be passed to PHYSFS_mount) for accessing the
 *  memfile archive.
 *
 *   \param memfile Pointer to a valid memfile object.
 *  \return READ-ONLY string representation of the memfile pointer for use as
 *          a PHYSFS archive name.
 */
__EXPORT__ const char *PHYSFS_getMemfileName(const PHYSFS_memfile *memfile);
