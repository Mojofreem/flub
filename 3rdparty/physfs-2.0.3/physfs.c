...
typedef struct __PHYSFS_DIRHANDLE__
{
    ...
    PHYSFS_memfile *memfile; /* Ptr to PHYSFS memfile. */
...
extern const PHYSFS_ArchiveInfo    __PHYSFS_ArchiveInfo_MEM;
extern const PHYSFS_Archiver       __PHYSFS_Archiver_MEM;
...
#endif
#if (defined PHYSFS_SUPPORTS_MEM)
...
static DirHandle *openDirectory(const char *d, int forWriting)
{
    ...
    if (ext != NULL)
    {
        logf("Looking for extension [%s]", ext);
        /* Look for archivers with matching file extensions first... */
        for (i = archivers; (*i != NULL) && (retval == NULL); i++)
        {
            logf("Checking [%s]...", (*i)->info->description);
...
const char *PHYSFS_getMemfileName(const PHYSFS_memfile *memfile)
{
    char *name;
    int size;
...
int PHYSFS_mount(const char *newDir, const char *mountPoint, int appendToPath)
{
    ...
    logf("Mounting [%s] at [%s]", newDir, mountPoint);
